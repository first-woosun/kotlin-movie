# kotlin-movie

# 3단계
## 기능 요구 사항
데이터베이스를 활용하여 영화 상영 정보와 예매 정보를 저장하고 조회할 수 있는 프로그램을 구현한다.

- 영화 상영 정보와 예매 정보는 데이터베이스에 저장한다.
- 프로그램을 다시 실행하더라도 저장된 영화 상영 정보와 예매 정보를 조회할 수 있어야 한다.
- 좌석 배치의 크기는 고정한다.
  - 상영관마다 좌석 수나 좌석 배치가 다른 경우는 고려하지 않는다.

## 프로그래밍 요구 사항

- 데이터베이스를 적용하더라도 기존 도메인 객체의 변경을 최소화한다.
- 데이터베이스 연동이 추가되더라도 핵심 비즈니스 로직의 역할과 책임이 흐려지지 않도록 구현한다.
- 데이터베이스 연동과 관련된 테스트를 작성한다.

## 기능 구현 목록
- [x] H2 데이터베이스 의존성 추가
- [ ] JdbcConnector 구현
  - [ ] 로컬 환경: 파일 기반 DB 연결
  - [ ] 테스트 환경: 인메모리 DB 연결

- [ ] 애플리케이션 시작 시 스키마 초기화
  - [ ] MOVIE 스키마 구현
    - [ ] 최초 실행시  초기 데이터 정의
    - [ ] 이후 실행시 기존 데이터 조회
  - [ ] SCREENING_SCHEDULE 스키마 구현
    - [ ] 최초 실행 시 초기 데이터 정의
  - [ ] RESERVATION 스키마 구현
  - [ ] RESERVED_SEAT 스키마 구현

- [ ] 영화 및 상영 일정 데이터 조회
  - [ ] MovieRepository: DB에서 전체 영화 목록 및 상세 정보 조회 및 객체 생성
  - [ ] ScheduleRepository: 특정 영화 또는 특정 날짜의 상영 일정 조회 및 객체 생성
  - [ ] TimeTableRepository: Mock 데이터 사용 대신 DB에 저장된 데이터를 조회 및 객체 생성

- [ ] 예매 정보 저장 및 관리
  - [ ] 선택한 좌석의 예약 가능 상태를 DB에서 조회
  - [ ] 예매 정보를 DB에 저장할 ReservationRepository
    - [ ] RESERVATION 테이블에 예매 정보 저장
      - [ ] 시간이 겹치는 예매가 있는지 검증
    - [ ] RESERVED_SEAT 테이블에 선택된 좌석의 정보 저장
      - [ ] 이미 예매된 좌석인지 검증

- [ ] 기존 코드와 DB 연동
  - [ ] MockTimeTable을 DB에 저장된 데이터로 교체
  - [ ] 프로그램 재시작 후에도 데이터 유지

## 테스트 구현 목록

1. 스키마 초기화 및 테스트
   - [ ] 프로그램을 실행하면 MOVIE, SCREENING_SCHEDULE, RESERVATION, RESERVED_SEAT 테이블이 생성된다
   - [ ] 최초 데이터 생성 후 프로그램을 재실행하면 기존에 저장된 데이터가 불러와진다

2. MovieRepository 테스트
   - [ ] DB에 저장된 모든 영화 정보가 Movie 객체 리스트로 반환된다
   - [ ] 특정 제목을 통해 영화의 상세 정보를 조회할 수 있다

3. ScheduleRepository 테스트
   - [ ] 특정 날짜를 입력했을 때 해당 날짜에 상여되는 모든 스케쥴이 반환된다
   - [ ] 특정 제목을 통해 해당 영화의 모든 상영 회차가 조회된다.

4. ReservationRepository 테스트
   - [ ] Reservation 객체를 생성한 후 DB에 저장하면 정상적으로 저장된다
   - [ ] 새로운 Reservation 객체의 상영 시간이 기존에 저장된 예매 시간과 겹치는지 확인할 수 있다
   - [ ] 이미 예매된 좌석을 조회할 수 있다
   - [ ] 새로운 좌석을 예매하면 DB에 해당 예매가 반영된다.

# 1단계

## 기능 요구 사항

한 영화관에서 쉽고 빠르게 영화를 예매할 수 있는 프로그램을 구현한다.

- 하나의 영화는 주어진 상영 가능 기간에 하루에도 여러 차례 상영될 수 있다.
- 상영 시간은 영화의 상영 길이와 상영관 운영 시간에 따라 정해진다.
- 하나의 영화가 여러 상영관에서 동시에 상영되는 상황은 고려하지 않는다.
- 여러 영화를 한 번에 예매할 수도 있다. 이때 서로 시간이 겹치는 상영은 함께 예매할 수 없다.
- 사용자는 영화와 상영 시간을 고른 뒤 원하는 좌석을 선택할 수 있다.
- 각 좌석은 행(알파벳)과 열(숫자)로 표시된다. 예를 들어 A1, C3, E4와 같이 나타낸다.
- 좌석은 등급에 따라 가격이 다르다.
    - S석은 18,000원
    - A석은 15,000원
    - B석은 12,000원
- 이미 예약된 좌석은 다시 선택할 수 없다.
- 특정한 조건을 만족하는 예매자는 추가 혜택을 받을 수 있다.
    - 무비데이 조건: 매월 10일, 20일, 30일에 상영되는 영화는 10% 할인된다.
    - 시간 조건: 오전 11시 이전 또는 오후 8시 이후에 시작하는 상영은 2,000원이 할인된다.
    - 두 조건이 동시에 적용될 수 있으며, 무비데이 할인(비율)이 먼저 적용되고, 이어서 시간 조건 할인(정액)이 적용된다.
- 사용자는 보유한 포인트를 사용할 수 있다. 포인트는 예매 금액에서 직접 차감된다.
- 포인트를 적용한 뒤 결제 수단에 따른 추가 할인이 적용된다.
    - 신용카드 결제는 5% 할인
    - 현금 결제는 2% 할인

## 구현할 기능 목록

### 1. movie

1. TITLE
    - [x] 영화의 이름은 빈 문자열이 될 수 없다.
    - [x] 입력받은 영화 제목이 Title이 갖고 있는 문자열과 동일하다면 true를 반환한다.
2. RunningTime
    - [x] 영화의 러닝 타임은 0보다 커야한다.
3. screening period
    - [x] 영화의 상영 종료일은 상영 시작일보다 빠를 수 없다.
    - [x] 입력 받은 날자가 영화의 상영 기간에 포함된다면 true가 반환된다.
    - [x] 입력 받은 날자가 영화의 상영 기간에 포함되지 않는다면 false가 반환된다.

### 2. ScreenTime

1. ScreeningTime

- [x] 입력받은 시간이 startTime과 endTime 사이에 있다면 true를 반환한다.
    1. startTime
        - [x] 입력된 시간이 소유하고 있는 시간과 같다면 true를 반환한다.
        - [x] 입력받은 시간이 소유하고 있는 시간과 다르다면 false를 반환하다.
    2. endTime

2. ScreeningDate
    - [x] 입력받은 날자가 소유하고 있는 날자와 같다면 true를 반환한다.
    - [x] 입력받은 날자가 소유하고 있는 날자와 다르다면 false를 반환하다.

### 3. screening schedule

- [x] 입력된 영화 제목이 소유하고 있는 영화 제목과 같다면 true를 반환받는다.
- [x] 입력된 영화 제목이 소유하고 있는 영화 제목과 다르다면 false를 반환받는다.
- [x] 입력된 상영 일자가 소유하고 있는 영화 상영 일자와 같다면 true를 반환받는다.
- [x] 입력된 상영 일자가 소유하고 있는 영화 상영 일자와 다르다면 false를 반환받는다.
- [x] 입력된 좌석 번호가 reservedSeats에 있다면 true를 반환한다.
- [x] 입력된 좌석 번호가 reservedSeats에 없다면 false를 반환한다.

### 4. timetable

- [x] 영화 제목을 입력받아 입력값과 같은 영화 제목을 갖는 screening schedule의 목록을 TimeTable 형태로 반환한다.
- [x] 입력받은 영화 제목이 screening schedule의 목록 중 일치하는 스케쥴이 없다면 예외를 발생시킨다.
- [x] 상영 일자를 입력받아 입력값과 같은 상영 일자를 갖는 screening schedule의 목록을 TimeTable 형태로 반환한다.
- [x] 입력받은 상영 일자가 screening schedule의 목록 중 일치하는 스케쥴이 없다면 예외를 발생시킨다.

### 5. screen

- [x] 입력된 좌석 번호가 상영관에 존재하는 좌석이면 true를 반환받는다.
- [x] 입력된 좌석 번호가 상영관에 존재하지 않는 좌석이면 false를 반환받는다.

### 6. seat

- [x] 입력된 좌석 번호가 저장된 좌석 번호와 일치하면 true를 반환한다.
- [x] 입력된 좌석 번호가 저장된 좌석 번호와 일치하지 않으면 false를 반환한다
- [x] 좌석의 등급에 따라 다른 가격을 반환한다.

1. SeatNumber
    1. RowNumber
        - [x] 입력된 행 번호가 자신의 번호와 같다면 true를 반환한다.
        - [x] 입력된 행 번호가 자신의 번호와 같지 않다면 false를 반환한다.
    2. ColumnNumber
        - [x] 입력된 열 번호가 자신의 번호와 같다면 true를 반환한다.
        - [x] 입력된 열 번호가 자신의 번호와 같지 않다면 false를 반환한다.
2. SeatGrade
    - [x] 지정된 랭크에 따라 좌석의 가격을 반환한다.

### 7. reservation

- [x] 입력된 상영 일자가 screenTime의 screeningDate과 같다면 true를 반환받는다.
- [x] 입력된 상영 시간이 screenTime의 ScreeningTime과 같다면 true를 반환받는다.

### 8. reservations

- [x] reservation 목록 중 입력받은 screenTime과 겹치는 reservation이 있다면 true를 반환받는다.
- [x] reservation 목록 중 입력받은 screenTime과 겹치는 reservation이 없다면 false를 반환받는다.
- [x] reservation 목록 중 입력받은 screenDate와 겹치는 reservation이 있다면 true를 반환받는다.
- [x] reservation 목록 중 입력받은 screenDate와 겹치는 reservation이 없다면 false를 반환받는다.

### 9. point

- [x] 포인트 초기값이 0보다 작으면 예외가 발생한다.
- [x] 사용할 포인트를 입력 받을 때 입력받은 값이 0보다 작으면 에러 메시지를 생성한다.
- [x] 사용할 포인트를 입력 받을 때 입력받은 값이 잔여 포인트보다 많으면 에러 메시지를 생성한다.
- [x] 입력받은 값이 0보다 크고 잔여 포인트보다 작으면 잔여 포인트에서 입력값 만큼 차감한다.

### 10. pay calculator

- [x] 입력받은 reservation 목록에 시간 관련 할인 정책을 적용한 결제 금액을 반환 받는다.
- [x] 포인트 사용을 적용한 결제 금액을 반환받는다.
- [x] 결제 수단 관련 할인 정책을 적용한 결제 금액을 반환 받는다.

### 11. discount policy

1. timeDiscountPolicy
    - [x] 예약 시간이 오전 11이전 또는 오후 8시 이후라면 2,000 할인한 금액을 반환한다.
2. movieDayDiscountPolicy
    - [x] 예약 날짜가 10, 20, 30일 중 하나라면 10% 할인한 금액을 반환한다.
3. cardDiscountPolicy
    - [x] 카드 결제시 5% 할인한 금액을 반환한다.
4. cashDiscountPolicy
    - [x] 현금 결제시 2% 할인한 금액을 반환한다.

### 12. Money

- [x] 입력된 값이 0보다 작으면 예외를 던진다.

# 2단계

## 기능 요구 사항

사용자가 콘솔에서 영화 상영을 조회하고 좌석을 선택하여 예매할 수 있는 콘솔 애플리케이션을 구현한다.

## 구현할 기능 목록

### 1. InputView

- [x] 예매 시작 여부를 입력받는다.
- [x] 영화 제목을 입력 받는다.
- [x] 날짜를 입력받는다.
- [x] 상영 번호를 입력받는다.
- [x] 예약할 좌석을 입력받는다.
- [x] 예매 추가 여부를 입력받는다.
- [x] 사용할 포인트를 입력받는다.
- [x] 사용할 결제 수단을 입력받는다.
- [x] 최종 결제 여부를 입력받는다.

### 2. OutputView

- [x] 영화의 상영 일정을 출력한다.
- [x] 상영관의 좌석 배치도를 출력한다.
- [x] 추가한 예매의 정보를 출력한다.
- [x] 최종 예매 목록을 출력한다.
- [x] 최종 결제 금액을 출력한다.
- [x] 최종 결제 내역을 출력한다.

### 3. Controller

- [x] 영화 예매를 시작한다.
- [x] 영화 제목과 날짜를 입력받아 상영 목록을 출력한 후 상영 번호를 선택한다.
- [x] 좌석 배치도를 출력하고 예약할 좌석번호를 입력받아 장바구니에 추가한다.
- [x] 다른 영화를 추가할지 입력받는다.
- [x] 장바구니에 입력된 예매 목록을 출력한다.
- [x] 사용할 포인트를 입력받고 결제 수단을 입력받아 최종 결제 금액을 계산한다.
- [x] 결제 후 최종 결제 내역을 출력한다.

## 1차 피드백
1. [x] DiscountAmount가 따로 빠져 있는데, 응집도 있는 객체라고 부를 수 있는가?
2. [x] 시간 할인 전략에서도 할인 조건이 바뀔 수 있다면 유연하게 대처할 수 있는 구조인가?
3. [x] Title의 isSame 메서드가 정말 필요한 메서드인가?
4. [x] Movie의 isValidTitle 메서드: 영화에게 영화의 제목이 유효한지 물어봐야 하는 이유가 무었인가?
5. [x] PayCalculator의 각 전략들이 런타임에 계속해서 변화되어야 한다면, 유연하게 대처할 수 있는 구조인가?
6. [x] PayCalculator Point 관련
   1. [x] 포인트를 매우 많이 쓰면 앱이 터지고, 에러 로그로 돈은 양수여야 한다고 나옴
   2. [x] 실제로 totalPrice에 point가 사용되었는가?
   3. [x] Point라는 객체가 있는데, usePoint 메서드에선 Money를 쓰고 있음. 프로젝트의 일관성이 유지되고 있는가?
7. [x] Reservations에서 date와 time을 나눠서 사용한 이유가 있는가?
8. [x] 좌석의 등급에서 상수를 따로 나누고, interface-class 구조로 만들 필요가 있었는가?
